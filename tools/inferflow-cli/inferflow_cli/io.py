"""Filesystem helpers for inferflow CLI."""
...
from inferflow_cli.models import InferflowConfig
...
def load_inferflow_config(repo_root: Path) -> InferflowConfig:
    """Load and validate inferflow.yaml from repository root.

    :param repo_root: Repository root path.
    :returns: Parsed project configuration.
    :raises FileNotFoundError: If inferflow.yaml does not exist.
    :raises ValueError: If the file is invalid.
    """
    config_path = repo_root / "inferflow.yaml"
    if not config_path.exists():
        raise FileNotFoundError("inferflow.yaml not found")

    raw = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    try:
        return InferflowConfig.model_validate(raw)
    except ValidationError as exc:  # pragma: no cover - formatting branch
        raise ValueError(f"Invalid inferflow.yaml: {exc}") from exc


def write_default_inferflow_config(repo_root: Path, name: str, registry: str) -> None:
    """Write a default inferflow.yaml file.
    ...
    target = repo_root / "inferflow.yaml"
